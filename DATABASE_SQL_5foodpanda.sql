...
-- Câu 8:
select firstname, lastname, orders_fooditem.orders_id,date_of_oders,sum(quantity) as 'total food',sum(quantity*unit_price) as Totalprice
from orders_fooditem,food_item,customer,orders where  orders_fooditem.fooditem_id = food_item.fooditem_id
and customer.customer_id = orders.customer_id and orders.orders_id = orders_fooditem.orders_id
and year(date_of_oders) = '2022'
group by orders_id
having sum(quantity*unit_price) >= 35000;

-- câu 9:
select customer.customer_id, orders.orders_id, firstname, lastname, email, sum(quantity)
from customer, orders, orders_fooditem, address 
where customer.customer_id = orders.customer_id and orders.orders_id = orders_fooditem.orders_id 
and customer.address_id = address.address_id and city in ('Da Nang', 'Quang Nam', 'Hue')  
group by (orders_fooditem.orders_id) having sum(quantity) > 2;
