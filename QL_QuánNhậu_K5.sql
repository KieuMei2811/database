...
-- 7*** Viết thủ tục cho biết thông tin chi tiết don hang trong một khoảng thời gian bất kỳ
 delimiter 
...
END;
-- 8*** Viết thủ tục chèn dữ liệu vào bảng nhân viên 
delimiter //
...
delimiter ;
-- Function --
-- 9*** Viết hàm trả về số lượng khách hàng đặt bàn bất kỳ 
DELIMITER //
...
END //
delimiter ;
-- 10*** Viết hàm tính tổng lần đi nhậu nhiều nhất của khách hàng
delimiter //
CREATE DEFINER=`root`@`localhost` FUNCTION `lannhau`(makhachhang  varchar(6)) RETURNS int(11)
BEGIN
declare b int;
