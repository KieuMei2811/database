...
on Ban 
for each row
insert into luutru (maban,tenban,loaiban,ngay)
values (new.maban,new.tenban,new.loaiban,now());
delimiter ;
-- INDEX --
CREATE INDEX index_DM ON Danh_Muc(MaDM);
select TenDM,TenLoai,DVT,GiaBan,Danh_Muc.MaDM 
...
drop INDEX index_DM ON Danh_Muc;    
