...
FOR EACH ROW 
BEGIN 
    if new.Maloai is not null THEN
    insert into tblHangHoa (Mahang,Maloai,Tenhang,TT)
    values (new.Mahang, New.Maloai, New.Tenhang,CONCAT('DA HOAN THANH',New.TT,'Please'));
    END IF ;
END $$
DELIMITER ;
 
 INSERT INTO tblHangHoa values ('K05',NULL,'Water','DA HOAN THANH');
                               
 drop trigger insert_TB;
-- C2: Tạo 1 Trigger: Nếu cập nhật vào trường maloai mà mã loại đó đang
-- tồn tại trong bảng HangHoa thì cập nhật luôn mã loại trong bảng HangHoa.
delimiter $$
CREATE TRIGGER CN_Maloai AFTER UPDATE 
ON Loai_Hang 
FOR EACH ROW  
BEGIN 
     UPDATE Hang_Hoa 
     SET New.Maloai = Hang_Hoa.old.Maloai ;
     End $$
delimiter ;     
